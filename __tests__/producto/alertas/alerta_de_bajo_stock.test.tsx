...
describe('Como empleado de una tienda de ropa, me gustaría poder visualizar un mensaje que me advierta del bajo stock', () => {
  test('Muestra alerta cuando hay productos con stock bajo', () => {
    const { getByText } = render(<AlertaBajoStock totalBajoStock={3} umbralBajoStock={10} />);

    expect(getByText('Alerta de Bajo Stock')).toBeTruthy();
    expect(getByText('Tienes 3 productos con stock bajo (≤ 10 unidades)')).toBeTruthy();
  });

  test('Muestra mensaje cuando solo hay un producto con stock bajo', () => {
    const { getByText } = render(<AlertaBajoStock totalBajoStock={1} umbralBajoStock={10} />);

    expect(getByText('Tienes 1 producto con stock bajo (≤ 10 unidades)')).toBeTruthy();
  });

  test('No muestra alerta cuando no hay productos con stock bajo', () => {
    const { queryByText } = render(<AlertaBajoStock totalBajoStock={0} umbralBajoStock={10} />);
