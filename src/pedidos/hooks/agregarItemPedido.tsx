...
export function agregarItemPedido() {
  ...
  const agregar = async (pedidoId: string, productoId: string, cantidad: number, precioUnitario: number) => {
    try {
      setLoading(true)
      setError(null)
      ...
      const { error: updateError } = await supabase.from("pedidos").update({ total: nuevoTotal }).eq("id", pedidoId)
